...
(* Bc1 -> Bc gamma: denominator reduction for projected G2 numerators.

   This script reduces the projected dimension-4 G2 numerators in terms of
   inverse propagator denominators.  It is the algebraic bridge between the
   FeynCalc traces and the derivative/contact double-Borel integration.
*)
...
ClearAll[
  ...
  d1, d2, d3, p2, pq,
  gammaA, gammaB, gammaP, S0Num, SG2Num, SGNum, GGVacuumTensor,
...
reduceExpr[expr_, topology_] := Module[{rules},
  ...
  Collect[Expand[e1Project[expr] /. rules], {d1, d2, d3}]
];
...
outFile = FileNameJoin[{outDir, "step3_ps_g2_denominator_reduction.csv"}];
...
Print["Wrote denominator-reduced G2 rows: ", Length[allRows]];
