#!/usr/bin/env python3
"""Fold explicit all-positive Bc gamma G2 shifts into pseudoscalar MC samples."""
...
def main() -> None:
    ...
    mc = read_csv(OUT / "bc_ps_complete_monte_carlo.csv")
    g2_grid = read_csv(OUT / "bc_ps_g2_all_positive_direct_borel_grid.csv")
    ratios = {
            ...
            rows.append(
                {
                    ...
                    "g_pert_plus_G2ap_GeV_inv": g_pert * (1.0 + delta),
                    "Gamma_pert_plus_G2ap_keV": gamma_pert * (1.0 + delta) ** 2,
                }
            )
    write_csv(OUT / "bc_ps_g2_all_positive_monte_carlo.csv", rows)
    summary_rows = []
    for channel in sorted({r["channel"] for r in rows}):
        sub = [r for r in rows if r["channel"] == channel]
        for obs in ["delta_g_over_g_G2ap", "g_pert_plus_G2ap_GeV_inv", "Gamma_pert_plus_G2ap_keV"]:
            summary_rows.append({"channel": channel, "observable": obs, **stats([float(r[obs]) for r in sub])})
    write_csv(OUT / "bc_ps_g2_all_positive_monte_carlo_summary.csv", summary_rows)
    lines = [
    ...
    for row in summary_rows:
        if row["observable"] == "Gamma_pert_plus_G2ap_keV":
            lines.append("{channel}: Gamma(pert + G2_ap) = {median:.4g} [{q16:.4g}, {q84:.4g}] keV".format(**row))
        if row["observable"] == "delta_g_over_g_G2ap":
            lines.append("{channel}: delta g/g = {median:.4g} [{q16:.4g}, {q84:.4g}]".format(**row))
    (OUT / "bc_ps_g2_all_positive_monte_carlo_summary.txt").write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
