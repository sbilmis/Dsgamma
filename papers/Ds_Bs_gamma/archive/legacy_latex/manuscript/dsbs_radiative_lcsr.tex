...
rules, while for the bottom-strange channel we use Pullin--Zwicky basis-current
inputs followed by a mixed-current diagonalization closure.  The preferred
lattice-normalized results give a
...
\begin{align}
  ...
  \label{eq:stage3}
\end{align}
The residue model is calibrated with the working Stage-3 anchor
$f_1=0.345\pm0.017~\GeV$; the same two-point overlap model predicts
$f_2\simeq0.38~\GeV$.  As a verification, we compare the mixed-current model
with the pure axial-current local-QCDSR benchmark
$f_A(D_{s1})=0.245\pm0.017~\GeV$ from Wang's Table~1 \cite{Wang2015}.  Since
Wang uses the pure axial current $J_A$, the direct comparison is only to our
limiting-current cases: $J_1=J_A$ at $\theta=90^\circ$ or $J_2=J_A$ at
$\theta=0^\circ$.  Our working pure-axial input
$f_A=0.225\pm0.025~\GeV$ is compatible with Wang's value within uncertainties.
At the physical angle, the Wang-like target requires a strongly negative
$AA/BB$ overlap and is therefore treated as a diagnostic benchmark, not as a
direct determination of the physical mixed residues.  The comparison is
summarized in Table~\ref{tab:wang-residue-comparison}.
For the bottom-strange channel we use the Pullin--Zwicky basis-current inputs,
$f_{\Bsone,A}^{\rm PZ}=0.305\pm0.027~\GeV$ and
$f_{\Bsone}^{T,\rm PZ}=0.285\pm0.027~\GeV$ \cite{PullinZwicky2021}.  We then
obtain the physical-current decay constants from a mixed-current
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llll}
    ...
    charm physical residues & $f_1,f_2$
      & $0.345(17),\,\simeq0.38~\GeV$ & working Stage-3 anchor and predicted partner\\
    pure axial benchmark & $f_A(D_{s1})$
      & $0.245(17)~\GeV$ & local-QCDSR comparison only\\
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lllllll}
    ...
    \midrule
    Wang benchmark & pure $J_A$ two-point current & -- & -- & $0.245(17)$ & -- & --\\
  ...
  \caption{Pure axial-current benchmark and limiting cases of the mixed-current
  residue model.  Wang's pure axial result is
  $f_A(D_{s1})=0.245\pm0.017~\GeV$; it tests the $J_A$ scale through the
  pure-current limits, while the physical mixed-current residues require an
  assumption about $\rho_{AB}$.  All listed residues are in GeV.}
...
\begin{figure}[htbp]
  ...
  photon-normalization scenario (left) and the bottom-strange physical-current
  normalization (right).}
  \label{fig:mc-widths}
\end{figure}
...
For $\Bsone(5830)$, there is no direct radiative measurement.  Our
physical-current-normalized prediction is a future-search target.  In the
preferred lattice-normalized scenario the median width remains below $1~\keV$,
although the mixing-angle scan leaves a long upper tail.
