...
This replacement is numerically important because the traditional
susceptibility input has a comparatively large uncertainty and appears through
the product $\chi_s\langle\bar s s\rangle$.  The lattice determination fixes
the leading-twist normalization more directly and reduces this part of the
uncertainty, although the cancellation-sensitive channels remain dominated by
the relative amplitude and mixing-angle dependence.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{../figures/figure1_borel_form_factors/figure1_borel_form_factors_mathematica.pdf}
\caption{Borel-parameter dependence of the transition form factors for
$\Dsone(2460)\to\Ds\gamma$, $\Dsone(2536)\to\Ds\gamma$,
$\Bsone(5750)\to\Bs\gamma$, and $\Bsone(5830)\to\Bs\gamma$.
For each channel, representative values of the continuum threshold $s_0$ are
shown within the working intervals of Table~\ref{tab:windows}.}
\label{fig:stability}
\end{figure}

The mixing-angle sensitivity is shown in
Fig.~\ref{fig:angle-dependence}.  The lower states remain comparatively stable
over the scan interval, whereas the higher states are more sensitive because
their two mixed-current contributions can nearly cancel.

\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{../figures/figure2_mixing_angle_widths/figure2_mixing_angle_widths_mathematica.pdf}
\caption{Dependence of the radiative decay widths on the mixing angle
$\theta$.  The charm-strange channels are shown in the left panel and the
bottom-strange channels in the right panel.  The heavy-quark-limit value
$\theta=35.3^\circ$ is indicated by a vertical dashed line.}
...
The suppression pattern can be seen directly at the amplitude level.
Table~\ref{tab:amplitude-decomposition} lists the two signed components
entering the physical form factor.  The lower-state components have the same
sign and add constructively, while the higher-state components have opposite
signs and are cancellation sensitive.  This destructive interference explains
the strong suppression of $\Dsone(2536)\to\Ds\gamma$ and
$\Bsone(5830)\to\Bs\gamma$, and provides the LCSR realization of the
cancellation mechanism discussed in Refs.~\cite{Bondar:2023mua,Bondar:2025smw}.

\input{../figures/figure3_amplitude_decomposition/amplitude_decomposition_table.tex}

\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{../figures/figure5_monte_carlo_widths/figure5_monte_carlo_widths_mathematica.pdf}
\caption{Monte Carlo width distributions obtained with the preferred
lattice-normalized photon input.  The scan varies the QCD and hadronic input
parameters, the Borel parameter $M^2$, the continuum threshold $s_0$, accepted
two-point residue samples, and the mixing angle over
$25^\circ\leq\theta\leq45^\circ$.  Solid vertical lines denote the median and
dashed vertical lines denote the $16$--$84$ percentile interval.}
\label{fig:mc}
\end{figure}
...
\begin{align}
  {\cal A}(u)
  &=
  40u^2\bar u^2\left(3\kappa-\kappa^+ +1\right)
  ...
  +2\bar u^3(10-15\bar u+6\bar u^2)\ln\bar u
  \Big],
  \label{eq:app-A}\\
  {\cal B}(u)
  &=
...
\begin{align}
  ...
  {\cal B}(u_0)
  &=
  -4\int_0^{u_0}du\,(u_0-u)h_\gamma(u),
...
of Ref.~\cite{Colangelo:2005hv}.  Thus
$\mathbb A-8\bar H_\gamma={\cal A}+2{\cal B}$.
