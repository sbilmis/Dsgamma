...
This script implements the hard-photon and two-particle-DA part of the
Colangelo-De Fazio-Ozpineci single-axial-current sum rule for the coupling g1
in D_{s1}(2460) -> D_s gamma, adapted to the bookkeeping agreed in this
project.
...
Transition-OPE schemes:
  - ``rohrwild_local_free`` (default): ordinary local condensates are not
    inserted into the photon LCSR.  The legacy heavy-charge local term is
    still returned as a diagnostic but is multiplied by zero in the OPE.
  - ``colangelo_local_benchmark``: reproduces the standalone heavy-charge
    condensate term printed by Colangelo et al.; this is a comparison mode,
    not the paper default.

The electromagnetic three-particle S_gamma and T4^gamma sector is not part of
Stage 1.  It requires a current-specific double-Borel reduction and must be
added as a separate Stage-2 contribution before the Rohrwild calculation can
be called complete.
...
def gauss_legendre_nodes(n: int) -> tuple[np.ndarray, np.ndarray]:
    """Return immutable-by-convention nodes reused throughout MC scans."""
...
def g1_stage1(
    ...
    *,
    ...
    transition_scheme: str = "rohrwild_local_free",
) -> dict[str, float | str]:
    ...
    if transition_scheme == "rohrwild_local_free":
        heavy_local = 0.0
    elif transition_scheme == "colangelo_local_benchmark":
        heavy_local = legacy_heavy_local
    else:
        raise ValueError(
            "transition_scheme must be 'rohrwild_local_free' or "
            "'colangelo_local_benchmark'"
        )
    ...
    return {
        ...
        "em_da_completion_status": "pending_current_specific_Sgamma_T4gamma",
        "M2": M2,
        "s0": s0,
        "g1_GeV_inv": g1,
        ...
        "legacy_heavy_local_diagnostic": legacy_heavy_local,
        "twist2": twist2,
        "twist4_2p": twist4_2p,
        "twist3_2p": twist3_2p,
    }
...
def run_grid(label: str, inputs: dict[str, float], *, transition_scheme: str):
    rows = []
    for s0_root in (2.50, 2.55, 2.60):
        ...
        for M2 in np.linspace(3.0, 6.0, 13):
            ...
            row["input_set"] = label
            row["transition_scheme"] = transition_scheme
            row["s0_root"] = s0_root
            rows.append(row)
    return rows
...
def run_grid_printed_colangelo_chi_sign(label: str, inputs: dict[str, float]):
    ...
    for s0_root in (2.50, 2.55, 2.60):
        ...
        for M2 in np.linspace(3.0, 6.0, 13):
            ...
            row["input_set"] = label
            row["transition_scheme"] = "colangelo_local_benchmark"
            row["s0_root"] = s0_root
            rows.append(row)
    return rows
...
def main():
    ...
    all_rows.extend(
        run_grid(
            "project_inputs_chi_positive_rohrwild_local_free_stage1",
            central_inputs(),
            transition_scheme="rohrwild_local_free",
        )
    )
    all_rows.extend(
        run_grid(
            "colangelo_inputs_chi_positive_local_benchmark_stage1",
            colangelo_inputs(),
            transition_scheme="colangelo_local_benchmark",
        )
    )
    ...
    summary_lines = [
        ...
        "Paper default: Rohrwild local-free transition OPE.",
        "The S_gamma/T4^gamma electromagnetic three-particle sector is pending",
        "its current-specific double-Borel reduction and is not included here.",
        summarize(
            [
                r
                for r in all_rows
            ...
            "Project inputs, chi=+3.15, local-free",
        ),
        summarize(
            [
                r
            ...
            "Colangelo-like inputs, chi=+3.15, local benchmark",
        ),
        summarize(
