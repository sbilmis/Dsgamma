...
def evaluate_components(M2=4.5, s0_root=2.55):
    ...
    gb_soft = gb_soft_two_particle(axial2, inputs, fB)
    gb_3p = gb_three_particle_from_integral(axial2, inputs, fB, f1_tensor)
    gb_em, _ = gb_em_da(axial2, inputs, fB)
    ga_stage1 = axial1["g1_GeV_inv"]
    ga_3p = axial2["F1_delta_g1"]
    return {
        "GA_stage1": ga_stage1,
        "GA_3p": ga_3p,
        "GA_em": axial2["em_delta_g1"],
        "GA_total": axial2["g1_stage2_GeV_inv"],
        "GB_hard": gb_hard,
        "GB_soft2p": gb_soft,
        "GB_3p": gb_3p,
        "GB_em": gb_em,
        "GB_total": gb_hard + gb_soft + gb_3p + gb_em,
    }
...
def contribution_breakdown_plot():
    ...
    pieces = [
        ...
        ("A EM 3p", comp["GA_em"], math.sin(theta) * comp["GA_em"], math.cos(theta) * comp["GA_em"]),
        ...
        ("B EM 3p", comp["GB_em"], math.cos(theta) * comp["GB_em"], -math.sin(theta) * comp["GB_em"]),
    ]
...
def literature_comparison_plot():
    ...
    for i, row in enumerate(rows):
        ...
        ax.errorbar(
            ...
            xerr=np.array([[lo], [hi]]),
            fmt="o",
            color=COLORS["blue"] if row["method"].startswith("This work") else COLORS["gray"],
