...
from rohrwild_em_da import tensor_em_qcd
from step10_stage2_f1_split import f1_sigma_like, f1_xgamma_like, integrate_piece
from step11_tensor_xgamma_ratio_integral import (
    integrate_xgamma,
    kinematics_options,
...
def gb_em_da(axial_stage2_row, inputs, fB):
    ...
    include = axial_stage2_row.get("transition_scheme") == "rohrwild_nonlocal"
    qcd = em["em_qcd"] if include else 0.0
    return prefactor_b(axial_stage2_row["M2"], inputs, fB) * qcd, em
...
def main():
    ...
    p2_residue, pq_residue = kinematics_options(inputs)["heavy_residue_p2_physical_pq"]
    f1_xgamma_matched, _, _ = integrate_xgamma(
        lambda aq, aqb, v: tensor_xgamma_matched_piece(
    ...
    for s0_root in (2.50, 2.55, 2.60):
        ...
        for M2 in np.linspace(3.0, 6.0, 13):
            ...
            for label, fB in fB_options.items():
                GB_soft2p = gb_soft_two_particle(axial2, inputs, fB)
                GB_hard, hard_qcd = hard_tensor_gb(float(M2), s0, inputs, fB)
                GB_em, em_details = gb_em_da(axial2, inputs, fB)
                ...
                GB_sigma_matched = GB_hard + GB_soft2p + GB_3p_sigma + GB_em
                GB_full_matched = GB_hard + GB_soft2p + GB_3p_full_matched + GB_em
                GB_full_ratio = GB_hard + GB_soft2p + GB_3p_full_ratio + GB_em
                ...
                rows.append(
                    {
                        ...
                        "GB_hard": GB_hard,
                        "GB_soft2p": GB_soft2p,
                        "GB_em_Sgamma_T4gamma": GB_em,
                        "GB_em_qcd": em_details["em_qcd"],
                        "GB_em_kernel_total": em_details["em_kernel_total"],
    ...
    lines = [
        ...
        "The complete result includes vector f3gamma psi^v and EM Sgamma/T4gamma terms.",
    ]
