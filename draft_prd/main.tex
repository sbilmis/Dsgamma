...
$B_{s1}^{\rm low}\to\Bs\gamma$, and $\Bsone(5830)\to\Bs\gamma$
...
\[
\Gamma[B_{s1}^{\rm low}\to\Bs\gamma]
   =18.4^{+9.7}_{-5.9}~\keV,
\qquad
\Gamma[\Bsone(5830)\to\Bs\gamma]
   =0.34^{+0.69}_{-0.18}~\keV ,
\]
where the lower state is treated as a theoretical diagnostic with
$M_{B_{s1}^{\rm low}}\simeq5.75~\GeV$.  The results support a strong hierarchy
...
$\Bsone(5830)$ state and a lower $B_{s1}^{\rm low}$ configuration.
...
bottom-strange sector, the lower $B_{s1}^{\rm low}$ state is treated as a
...
with momentum \(P=p+q\) into a pseudoscalar strange heavy meson with momentum
...
associates \(J_1\) with the lower diagnostic configuration \(B_{s1}^{\rm low}\)
...
\begin{align}
  ...
  \label{eq:fp-def}\\
  \langle0|J_{i\mu}|X_i(P,\eta)\rangle
  &=f_i m_i\eta_\mu ,
  \qquad i=1,2 ,
  \label{eq:fi-def}
\end{align}
...
\begin{equation}
  \langle P(p)\gamma(q,\varepsilon)|X_i(P,\eta)\rangle
  =
  e g_i
  \left[
...
The light-cone sum rule is obtained from the vacuum-to-photon correlation
function
\begin{equation}
...
matrix elements.  The photon emission from the light quark is represented by
the standard vacuum-to-photon matrix elements of nonlocal quark and
quark-gluon operators.  The corresponding two- and three-particle photon
distribution amplitudes are collected in Appendix~\ref{app:sumrule-expressions}.
Terms proportional to \(m_s\) are kept throughout.
...
\begin{equation}
  ...
  \label{eq:pi-decomp}
\end{equation}
Here the three terms denote perturbative photon emission, two-particle
photon-DA contributions, and three-particle photon-DA contributions,
...
\(B_{s1}^{\rm low}\to\Bs\gamma\), while the second is used for
...
The physical-current residues are not independent external inputs.  For the
charm-strange sector we use $f_1=0.345(17)~\GeV$ as the working normalization
anchor and obtain $f_2\simeq0.38~\GeV$ from the same mixed-current overlap
model.  For the bottom-strange sector, the basis-current constants of
Ref.~\cite{Pullin:2021ebn} are used to fix the basis normalization; the
corresponding physical-current residues are then obtained by the mixed-current
diagonalization and quoted together with the final results.
...
\begin{ruledtabular}
\begin{tabular}{lll}
Channel & $M^2$ window & $\sqrt{s_0}$ window \\
\hline
$\Dsone(2460),\Dsone(2536)\to\Ds\gamma$
& $3.0$--$6.0~\GeV^2$
& $2.50$--$2.60~\GeV$ \\
$B_{s1}^{\rm low}\to\Bs\gamma$
& $8.0$--$14.0~\GeV^2$
& FILL \\
$\Bsone(5830)\to\Bs\gamma$
& $8.0$--$14.0~\GeV^2$
...
& $f_{\gamma,s}^{\perp}$
& $0.379^{+0.044}_{-0.041}$
& $+0.0403^{+0.0193}_{-0.0209}$
& $0.504^{+0.600}_{-0.377}$\\

$\Bsone(5830)\to\Bs\gamma$
& $\chi_s\langle\bar s s\rangle$
& $0.218^{+0.074}_{-0.080}$
& $-0.0793^{+0.0256}_{-0.0507}$
& $1.33^{+2.27}_{-0.72}$\\

$\Bsone(5830)\to\Bs\gamma$
& $f_{\gamma,s}^{\perp}$
& $0.203^{+0.088}_{-0.082}$
& $-0.0401^{+0.0127}_{-0.0294}$
& $0.342^{+0.684}_{-0.182}$\\
\end{tabular}
\end{ruledtabular}
\end{table}
...
For the lower bottom-strange diagnostic configuration we obtain
\begin{equation}
  \Gamma[B_{s1}^{\rm low}\to\Bs\gamma]
  =
  18.4^{+9.7}_{-5.9}~\keV
  \label{eq:bs-low-width}
\end{equation}
with the $f_{\gamma,s}^{\perp}$ normalization.  Since this configuration is
not an established experimental resonance, we quote it separately from the
primary predictions in Table~\ref{tab:results}.
...
\begin{equation}
  ...
  0.34^{+0.69}_{-0.18}~\keV .
\end{equation}
For the lower $B_{s1}^{\rm low}$ diagnostic configuration we find a larger
