...
\begin{equation}
  ...
  \label{eq:width}
\end{equation}
In Eq.~\eqref{eq:e1} the symbol $G$ denotes the physical transition form
factor of the state under discussion.  To avoid mixing this physical quantity
with the intermediate basis-current amplitudes, below we use
$\mathcal{G}_A$ and $\mathcal{G}_B$ only for the two unmixed basis-current
form factors, and $G_{2460}$, $G_{2536}$, $G_{5748}$, and $G_{5829}$ for
physical mesons.
...
\begin{equation}
  ...
  \frac{m_A f_A^{(i)}\mathcal{G}_i}
  {(m_A^2-P^2)(m_P^2-p^2)}
  \left[(p\cdot q)\varepsilon_\mu^*
  -(p\cdot\varepsilon^*)q_\mu\right]+\cdots ,
  \label{eq:hadronic}
\end{equation}
where $i=A,B$ for the basis currents, so
$\mathcal{G}_i=\{\mathcal{G}_A,\mathcal{G}_B\}$ at this intermediate stage.
The ellipsis denotes higher states and continuum contributions.
...
\begin{align}
  G_{2460} &=
  \frac{\sin\theta\,f_A\,\mathcal{G}_A
  +\cos\theta\,f_B\,\mathcal{G}_B}{f_1},\\
  G_{2536} &=
  \frac{\cos\theta\,f_A\,\mathcal{G}_A
  -\sin\theta\,f_B\,\mathcal{G}_B}{f_2}.
