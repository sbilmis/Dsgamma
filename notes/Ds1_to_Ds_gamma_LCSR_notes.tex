...
The entries in Table~\ref{tab:recommended-ds1-results} should be kept as the
completed Stage-1 plus Stage-2 \emph{basis-current normalization baseline}.
They should not be discarded, but the final paper should either quote them with
this label or replace the $G$ values by the physical-current normalized
couplings of Eqs.~\eqref{eq:physical-current-g1}--\eqref{eq:physical-current-g2}
after the two-point residues $f_1$ and $f_2$ are determined.
...
\begin{table}[htbp]
  ...
  inputs; a fully matched-scale prediction should be generated as a separate
  rerun.}
...
Table~\ref{tab:combined-ds-bs-summary} collects the current baseline width
predictions in one place.  The $D_{s1}$ rows are completed Stage-1 plus
Stage-2 basis-current normalization baselines; the final physical-current
normalization requires the two-point residues $f_1$ and $f_2$ discussed above.
The $B_{s1}$ rows are working baseline results because the axial and tensor
$B_{s1}$ decay constants are still temporary heavy-quark-scaled inputs.  All
entries are medians with 16--84 percentile intervals.
...
\begin{table}[htbp]
  ...
  Eq.~\eqref{eq:width}.  The $D_s$ rows are basis-current normalized baselines
  pending a two-point normalization update for $f_1$ and $f_2$.  The $B_s$ rows
  should be treated as working results until dedicated $B_{s1}$ decay constants
  replace the heavy-quark-scaled temporary inputs.}
...
The Stage-1 and Stage-2 three-point OPE calculation is complete at the level
needed to start writing.  Before presenting the numerical tables as final
physical-current predictions, however, the physical-current two-point
normalization should be completed:
\begin{itemize}
  \item evaluate the two-point sum rules for $J_1$ and $J_2$, using
  $\Pi_{AA}$, $\Pi_{BB}$ and $\Pi_{AB}$, and extract $f_1$ and $f_2$;
  \item update the headline $G$ and $\Gamma$ values from the basis-current
  baseline to the physical-current normalization if the extracted residues
  differ from the basis normalization;
