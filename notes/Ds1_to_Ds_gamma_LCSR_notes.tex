...
\begin{align}
  ...
  \right].
\end{align}
In the first candidate implementation we use the diagonal double-dispersion
identification $p^2\to s$ inside this numerator.  This is the remaining point
that should be independently checked from the double spectral representation.
...
final result, because Stage 2 and the independent double-spectral check of the
$p^2/(p\cdot q)$ term remain.
