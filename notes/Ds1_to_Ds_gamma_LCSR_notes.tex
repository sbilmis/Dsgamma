...
Using this ratio as a first estimate for the tensor-current
three-particle correction gives
\begin{align}
  \Delta G_B^{3p}
...
\end{align}
The resulting Stage 2 mixed-width estimate at $\theta=35.3^\circ$ is
\begin{align}
  \Gamma[D_{s1}(2460)\to D_s\gamma]
  &\simeq 61.9\text{ to }87.5\ \mathrm{keV},
  &
  \Gamma[D_{s1}(2536)\to D_s\gamma]
  &\simeq 0.00\text{ to }0.29\ \mathrm{keV},
  &&(f_B=f_T\ \text{diagnostic}),\\
  \Gamma[D_{s1}(2460)\to D_s\gamma]
  ...
  &&\left(f_B=f_T\frac{m_{D_{s1}}}{m_c+m_s}\ \text{central}\right).
\end{align}
These numbers are very close to the Stage 1 tensor-current candidate because
the three-particle correction is numerically small.  They should still be
labeled as an estimate until the explicitly $x$-dependent ${\cal T}_3$,
${\cal T}_4$, and $x_\alpha\gamma_\beta$ kernels are reduced for $J_B$.
...
\end{align}
All these reduced kernels carry the squared heavy-quark denominator, so they
are double-pole-type contributions and should survive the double Borel
projection.  Consequently the current Stage 2 tensor numerical result remains
an estimate: the remaining task is not another Dirac trace, but matching these
derivative-reduced kernels to the same normalization that produces the known
axial ${\cal F}_1$ combination, and then integrating the resulting tensor
combination over $(v,\alpha_g)$.
