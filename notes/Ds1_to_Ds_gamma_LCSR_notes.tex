...
\begin{table}[htbp]
  ...
  \begin{tabular}{llllcc}
    \toprule
    Target & Photon input & Mixing input & implied $\chi_s$
    & $G$ ($\GeV^{-1}$) & $\Gamma$ (keV)\\
    \midrule
    $\Bsone(5830)$ high combo
      & legacy $\chi_s$ (Rohrwild/BBK)
  ...
  \caption{Monte Carlo $B_{s1}\to B_s\gamma$ baseline.  Entries are medians
  with 16--84 percentile intervals.  The observed $\Bsone(5830)$ amplitude is
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{../outputs/bs1_5830_mc_width_histograms.pdf}
  \caption{Monte Carlo width distributions for the observed
  $\Bsone(5830)\to\Bs\gamma$ high-combination state.  The legacy and
  lattice-normalized photon inputs are shown separately for fixed
  $\theta=35.3^\circ$ and for the mixing-angle scan.}
...
range.  This makes the legacy scenario compatible with a sub-keV-to-few-keV
width, while the lattice-normalized leading-twist input shifts the median to
about $5~\mathrm{keV}$ but leaves a long upper tail.  A final quoted
$B_{s1}$ prediction therefore needs dedicated $B_{s1}$ decay constants before
we should advertise the central value too strongly.  The machine-readable
Monte Carlo outputs are \texttt{outputs/bs1\_stage2\_mc\_scan.csv} and
\texttt{outputs/bs1\_stage2\_mc\_summary.csv}.
...
normalization described above.  The $B_{s1}$ rows remain working baseline
results because the axial and tensor $B_{s1}$ decay constants are still
temporary heavy-quark-scaled inputs.  All entries are medians with 16--84
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lllllcc}
    ...
    $B_s$ working
      ...
      & legacy $\chi_s$ (Rohrwild/BBK)
      & $\theta=35.3^\circ$
      & working basis-current baseline
      & $+0.0273\,[-0.0592,+0.133]$
      & $1.04^{+3.41}_{-0.97}$\\
    $B_s$ working
      & $\Bsone(5830)\to\Bs\gamma$
  ...
  residue correction.  The $B_s$ rows should be treated as working results until
  dedicated $B_{s1}$ decay constants replace the heavy-quark-scaled temporary
  inputs.}
...
\begin{itemize}
  ...
  separate phenomenology subsection, because their decay constants are still
  working HQ-scaled inputs.
\end{itemize}
