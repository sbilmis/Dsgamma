...
predictions in one place.  The $D_{s1}$ rows are the current final baseline of
this note.  The $B_{s1}$ rows are working baseline results because the axial and
tensor $B_{s1}$ decay constants are still temporary heavy-quark-scaled inputs.
All entries are medians with 16--84 percentile intervals.
...
\begin{table}[htbp]
  ...
  Eq.~\eqref{eq:width}.  The $B_s$ rows should be treated as working results
  until dedicated $B_{s1}$ decay constants replace the heavy-quark-scaled
  temporary inputs.}
...
\begin{itemize}
  ...
  $G_{\rm high}=\cos\theta\,G_A-\sin\theta\,G_B$ in our convention.
...
The core calculation is complete at the level needed to start writing.  The
remaining tasks are editorial:
\begin{itemize}
