...
\begin{table}[htbp]
  ...
  \begin{tabular}{llll}
    ...
    \midrule
    $\Dsone(2460)\to\Ds\gamma$
      & legacy $\chi_s$: $36.7\,[26.8,50.2]~\mathrm{keV}$
      & $\mathcal B(\Dsone(2460)\to\Ds\gamma)=(16\pm4\pm3)\%$ (BaBar)
      & implies $\Gamma_{\rm tot}\sim0.23~\mathrm{MeV}$; compatible with a very narrow state\\
    $\Dsone(2460)\to\Ds\gamma$
      & lattice $f_{\gamma,s}^{\perp}$: $14.4\,[11.4,18.7]~\mathrm{keV}$
      & $\mathcal B(\Dsone(2460)\to\Ds\gamma)=(16\pm4\pm3)\%$ (BaBar)
      & implies $\Gamma_{\rm tot}\sim0.09~\mathrm{MeV}$; also compatible with a very narrow state\\
    $\Dsone(2536)\to\Ds\gamma$
      & legacy $\chi_s$: $5.60\,[2.56,10.98]~\mathrm{keV}$
      & $\Gamma_{\rm tot}(\Dsone(2536))=0.92\pm0.05~\mathrm{MeV}$ (BaBar)
      & implies $\mathcal B_\gamma\sim0.6\%$; experimentally small channel\\
    $\Dsone(2536)\to\Ds\gamma$
      & lattice $f_{\gamma,s}^{\perp}$: $0.68\,[0.12,1.88]~\mathrm{keV}$
      & $\Gamma_{\rm tot}(\Dsone(2536))=0.92\pm0.05~\mathrm{MeV}$ (BaBar)
      & implies $\mathcal B_\gamma\sim0.07\%$; strongly cancellation suppressed\\
    $\Bsone(5830)\to\Bs\gamma$
      & legacy $\chi_s$: $1.04\,[0.068,4.45]~\mathrm{keV}$
      & no direct radiative measurement; observed mainly in $B^{(*)}K$ spectroscopy
      & prediction for future searches; comparison is qualitative\\
    $\Bsone(5830)\to\Bs\gamma$
      & lattice $f_{\gamma,s}^{\perp}$: $5.30\,[0.454,18.9]~\mathrm{keV}$
      & no direct radiative measurement; observed mainly in $B^{(*)}K$ spectroscopy
      & prediction for future searches; branching fraction depends on the total width\\
    \bottomrule
  \end{tabular}%
  }
...
radiative branching fraction is expected to be below the percent level,
especially in the lattice-normalized photon-input scenario.  For
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llll}
    ...
    \midrule
    charm mass & $m_c$ & $1.27\pm0.02~\GeV$ & $\overline{\mathrm{MS}}$ input for $D_s$ sector\\
    bottom mass & $m_b$ & $4.18\pm0.03~\GeV$ & $\overline{\mathrm{MS}}$ input for $B_s$ sector\\
    strange mass & $m_s$ & $0.093\pm0.011~\GeV$ & retained explicitly, including $m_s^2$ terms\\
    $\Dsone(2460),\Dsone(2536),\Ds$ masses
      & $M_{2460},M_{2536},m_{\Ds}$
      & $2.4595,\;2.53511,\;1.96835~\GeV$
      & physical charm-strange masses\\
    $\Bsone(5830),\Bs$ masses
      & $M_{5830},m_{\Bs}$
      & $5.82870,\;5.36692~\GeV$
      & observed bottom-strange channel\\
    lower $\Bsone$ diagnostic mass
      & $M_{\Bsone}^{\rm low}$
      & $5.750\pm0.026~\GeV$
      & expected, not-yet-established lower $1^+$ partner\\
    $\Ds$ and $\Dsone$ decay constants
      & $f_{\Ds},f_{\Dsone,A},f^T_{\Dsone}$
      & $0.2499,\;0.225,\;0.256~\GeV$
      & hadronic-side inputs\\
    $\Dsone$ physical-current residues
      & $f_1,\;f_2$
      & $0.345\pm0.017,\;\simeq0.38~\GeV$
      & $f_1$ from local QCDSR; $f_2$ from Stage-3 overlap calibration\\
    $\Bs$ and working $\Bsone$ decay constants
      & $f_{\Bs},f_{\Bsone,A},f^T_{\Bsone}$
      & $0.2303,\;0.146,\;0.164~\GeV$
      & $\Bsone$ values are temporary HQ-scaled inputs\\
    PZ $\Bsone$ decay-constant scenario
      & $f_{\Bsone,A}^{\rm PZ},f_{\Bsone,T}^{\rm PZ}$
      & $0.305\pm0.027,\;0.285\pm0.027~\GeV$
      & dedicated bottom-sector input scenario\\
    condensates
      & $\langle\bar q q\rangle,\kappa$
      & $-(0.240~\GeV)^3,\;0.8\pm0.1$
      & $\langle\bar s s\rangle=\kappa\langle\bar q q\rangle$\\
    legacy photon input
      & $\chi_s(1~\GeV)$
      & $+3.15\pm0.30~\GeV^{-2}$
      & Rohrwild/BBK-style comparison scenario\\
    lattice photon input
      & $f_{\gamma,s}^{\perp}(1~\GeV)$
      & $-55.1\pm1.9~\mathrm{MeV}$
      & preferred leading-twist normalization scenario\\
    mixing angle
      & $\theta$
      & $35.3^\circ$ central, $25^\circ$--$45^\circ$ scan
      & ${}^3P_1$--${}^1P_1$ physical-state mixing\\
    $D_s$ sum-rule window
      & $M^2,\sqrt{s_0}$
...
The Stage-1 and Stage-2 three-point OPE calculation is complete, and the
$D_{s1}$ numerical table now includes a Stage-3 physical-current residue
normalization calibrated from the available local-QCDSR value of
$f_{D_{s1}(2460)}$.  Before the manuscript is frozen, the remaining physics
refinement is to replace this calibration by a fully independent local
two-point calculation of the complete current matrix:
\begin{itemize}
  ...
  \item build a proper BibTeX file and cite every input in
  Table~\ref{tab:paper-input-summary};
  ...
  two-point matrix remains a later refinement.
\end{itemize}
