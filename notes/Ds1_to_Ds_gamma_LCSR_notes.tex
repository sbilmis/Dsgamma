...
invariant, the matched integral is
\begin{equation}
  \int{\cal F}_{1,B}^{x\gamma}[{\cal S},{\cal T}_2,{\cal T}_3]
  =
  +2.5818\times 10^{-1}.
\end{equation}
Together with the sigma-like part this gives
\begin{equation}
  \int\left(
    {\cal F}_{1,B}^{\sigma}
    +
    {\cal F}_{1,B}^{x\gamma}[{\cal S},{\cal T}_2,{\cal T}_3]
  \right)
  =
  +1.0193\times 10^{-1}.
\end{equation}
...
There remains one important caveat.  The tensor-current derivative reduction
produces a ${\cal T}_4$ residue even though the axial E1 kernel has
$A_{{\cal T}_4}^{x\gamma}=0$.  The dimensionless diagnostic shape integral for
this tensor-only term is
\begin{equation}
  \int{\cal T}_4^{\rm diagnostic}
  =
  -4.2136\times 10^{-2},
\end{equation}
but this is not yet a properly normalized sum-rule contribution.  Therefore
the best current Stage 2 tensor estimate includes the sigma-like part plus the
matched ${\cal S},{\cal T}_2,{\cal T}_3$ $x\gamma$ part, while the
${\cal T}_4$ tensor-only residue remains the last analytic item.

Numerically, for the central decay-constant normalization
$f_B=f_Tm_{D_{s1}}/(m_c+m_s)$, the partial matched Stage 2 tensor-current
result gives
\begin{align}
  \Delta G_B^{3p}
  &=
  +0.00076\text{ to }+0.00132\ \mathrm{GeV}^{-1},\\
  G_B^{\rm Stage\ 2}
  &=
  -0.2818\text{ to }-0.2433\ \mathrm{GeV}^{-1}.
\end{align}
At $\theta=35.3^\circ$ this corresponds to
\begin{align}
  \Gamma[D_{s1}(2460)\to D_s\gamma]
  &=
  30.5\text{ to }44.5\ \mathrm{keV},\\
  \Gamma[D_{s1}(2536)\to D_s\gamma]
  &=
  4.0\text{ to }8.1\ \mathrm{keV},
\end{align}
before adding the unresolved ${\cal T}_4$ piece.
