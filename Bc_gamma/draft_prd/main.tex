...
rules, we find
\[
\Gamma[\Bcone(6743)\to \Bc\gamma]=0.223^{+0.084}_{-0.061}~\keV,\qquad
\Gamma[\Bcone(6750)\to \Bc\gamma]=8.01^{+1.89}_{-1.41}~\keV .
\]
For the vector final state, using the standard vector-current normalization
\(f_{\Bcst}=0.435^{+0.032}_{-0.035}\,\GeV\), we obtain the leading
hard-photon baselines
\[
\Gamma[\Bcone(6743)\to \Bcst\gamma]=74.3^{+11.2}_{-9.0}~\keV,\qquad
\Gamma[\Bcone(6750)\to \Bcst\gamma]=360^{+52}_{-48}~\keV .
\]
We also give a conservative dimension-four gluon-condensate screening
uncertainty and show that the vector-channel contact terms have no ordinary
two-channel double-pole support.
\end{abstract}
...
contact/derivative-sector terms.  A refined double-Borel support audit shows
that the contact/derivative rows do not contain ordinary two-channel double
poles.  The apparent 131 active rows all depend on the crossed combination
\((p-q)^2=2p^2-P^2\), and are therefore kept as audit candidates rather than
added to the physical double-Borel sum.  Until the all-positive \(G^2\) sector
is integrated explicitly, we quote a conservative systematic envelope obtained
from the same-window two-point \(G^2/{\rm pert}\) ratios.
...
\begin{table}[t]
\centering
\caption{Present hard-QCDSR results.  The first uncertainty is the Monte Carlo
input/Borel-window uncertainty.  The second uncertainty, where shown, is the
conservative \(G^2\) screening envelope.}
\begin{tabular}{lcc}
\toprule
...
& \(-0.0785[-0.0843,-0.0736]\)
& \(74.3^{+11.2}_{-9.0}\pm11.1_{G^2{\rm\,scr.}}\)\\
\(\Bcone(6750)\to \Bcst\gamma\)
& \(+0.168[0.156,0.180]\)
& \(360^{+52}_{-48}\pm54_{G^2{\rm\,scr.}}\)\\
\bottomrule
\end{tabular}
...
The dimension-four gluon-condensate sector has been reduced algebraically and
audited.  In the vector channel the contact/derivative rows do not have
ordinary two-channel double-pole support; they are crossed-contact audit
candidates.  The numerical results are therefore quoted as leading
perturbative hard-QCDSR predictions with a conservative \(G^2\) screening
uncertainty.  A future refinement can integrate the all-positive radiative
\(G^2\) terms explicitly, but the present calculation already gives a
transparent QCDSR baseline for comparison with model predictions and future
measurements of \(B_c(1P)\) radiative decays.
