...
\(f_{\Bcst}=0.435^{+0.032}_{-0.035}\,\GeV\), the baseline current convention gives
\[
...
opposite to representative model calculations, so the \(\Bcst\gamma\) entries
should be regarded as convention-dependent until an independent tensor-current
trace check is completed.
\end{abstract}
...
all-positive \(G^2\) sector.  The \(\Bcst\gamma\) entries are baseline
current-convention values and remain under the tensor-current sign audit
...
\end{table}
\noindent\(\dagger\) Baseline current-convention values; the vector hierarchy
is under the tensor-current sign and state-assignment audit.
...
More importantly, the baseline vector ratio
...
definition used in the two-point mixing analysis.  We therefore keep the vector
entries as a baseline convention, not as final physical predictions.
...
normalized, but its physical hierarchy remains under sign-convention audit.
...
\(\Bc\gamma\) and a documented baseline for \(\Bcst\gamma\).  Finalizing the
vector channel requires an independent rederivation of the tensor-current trace
and physical-state assignment.
