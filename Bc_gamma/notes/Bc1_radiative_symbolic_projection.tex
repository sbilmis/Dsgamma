...
The numerical table above contains only the leading hard-photon perturbative
baseline.  In practical terms this means:
\begin{itemize}
  \item the photon is emitted from the charm or anti-bottom heavy-quark line;
  \item both heavy propagators are free propagators in the numerical pilot;
  ...
  heavy propagators are not yet included;
...
\end{itemize}
Thus the present result is best described as a leading perturbative hard-QCDSR
pilot result.  A final publication-level \(B_c\) analysis should add, or
explicitly bound, the heavy-heavy gluon-condensate corrections and the contact
terms.
...
\begin{enumerate}
  ...
  \path{Bc_gamma/scripts/bc_radiative_pilot_sumrule.py}.  The script uses the
  diagonal calibration \(M_1^2=M_2^2=2M^2\), with
  \(M^2=\{7,8,9\}\,\GeV^2\) and \(s_0=\{53,54,55\}\,\GeV^2\).
  \item Use \(m_b=4.18\,\GeV\), \(m_c=1.27\,\GeV\), and the central mixing
  angle \(\theta=43.3^\circ\).  The current pilot uses
  \(f_{\Bc}=0.371\,\GeV\), \(f_{\Bcst}=0.384\,\GeV\), and a common axial-vector
