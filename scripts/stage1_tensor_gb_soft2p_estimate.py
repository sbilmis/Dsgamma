...
The vector DA contribution to J_B contains extra p^2/(p.q) kinematics and is
reported only as an optional diagnostic, not included in the default estimate.
The hard-photon tensor-current contribution is also not included here.
"""
...
def gb_vector_da_diagnostic(axial_row, inputs, fB, ubar=0.5):
    """Optional vector-DA B-current diagnostic from trace-ratio calibration.
    ...
    f3gamma*Psi^v OPE term.  Since the B trace contains p^2/(p.q), we insert
    physical kinematics here only as a diagnostic.
    """
    mc = inputs["mc"]
    ms = inputs["ms"]
    p2 = inputs["m_ds"]**2
...
def main():
    ...
    for s0_root in (2.50, 2.55, 2.60):
        ...
        for M2 in np.linspace(3.0, 6.0, 13):
            ...
            for label, fB in fB_options.items():
                ...
                GB_vec_diag = gb_vector_da_diagnostic(axial, inputs, fB)
    ...
    lines = [
        ...
        "=================================================",
        "Default includes tensor-DA family only: chi phi_gamma + A/h_gamma.",
        "Vector DA entry is diagnostic because it uses physical p^2/(p.q).",
