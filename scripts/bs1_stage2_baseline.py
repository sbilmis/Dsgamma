"""First matched Stage-2 baseline for Bs1 -> Bs gamma.
...
def evaluate_point(inputs: dict[str, float], M2: float, s0_root: float, theta_deg: float, f1_axial: float, f1_basis: dict[str, float]):
    ...
    GB_soft = gb_soft_tensor_da(axial, calc_inputs, fB)
...
def main():
    ...
    lines = [
        "Bs1 -> Bs gamma first matched Stage-2 baseline",
        "================================================",
        "Internal key mc is used as m_b in this script.",
        "Hard photon, two-particle DAs, and matched three-particle DAs are included.",
