...
benchmark; for the bottom-strange channel we use Pullin--Zwicky basis-current
...
$f_1^{(B_s)}=0.443[0.384,0.492]~\GeV$ and
$f_2^{(B_s)}=0.203[0.121,0.291]~\GeV$.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llll}
    ...
    bottom physical constants & $f_1^{(B_s)},f_2^{(B_s)}$
      & $0.443[0.384,0.492],\,0.203[0.121,0.291]~\GeV$ & this work\\
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lllllcc}
    ...
    $B_s$ & $\Bsone(5830)\to\Bs\gamma$ & legacy $\chi_s$
      & $35.3^\circ$ & $f_2^{(B_s)}=0.218[0.138,0.292]$
      & $-0.0793[-0.130,-0.0537]$ & $1.33^{+2.27}_{-0.72}$\\
    $B_s$ & $\Bsone(5830)\to\Bs\gamma$ & lattice $f_{\gamma,s}^{\perp}$
      & $35.3^\circ$ & $f_2^{(B_s)}=0.203[0.121,0.291]$
      & $-0.0401[-0.0695,-0.0274]$ & $0.342^{+0.684}_{-0.182}$\\
    $B_s$ & $\Bsone(5830)\to\Bs\gamma$ & legacy $\chi_s$
      & $25^\circ$--$45^\circ$ & $f_2^{(B_s)}=0.246[0.174,0.312]$
      & $-0.0380[-0.103,+0.0101]$ & $0.349^{+1.908}_{-0.323}$\\
    $B_s$ & $\Bsone(5830)\to\Bs\gamma$ & lattice $f_{\gamma,s}^{\perp}$
      & $25^\circ$--$45^\circ$ & $f_2^{(B_s)}=0.249[0.170,0.307]$
      & $+0.0137[-0.0612,+0.0657]$ & $0.493^{+1.239}_{-0.441}$\\
    $B_s$ & $\Bsone^{\rm low}(5750)\to\Bs\gamma$ & legacy $\chi_s$
      & $35.3^\circ$ & $f_2^{(B_s)}=0.226[0.126,0.303]$
      & $+0.284[+0.227,+0.360]$ & $9.50^{+7.45}_{-3.53}$\\
    $B_s$ & $\Bsone^{\rm low}(5750)\to\Bs\gamma$ & lattice $f_{\gamma,s}^{\perp}$
      & $35.3^\circ$ & $f_2^{(B_s)}=0.216[0.132,0.285]$
      & $+0.391[+0.331,+0.457]$ & $18.4^{+9.7}_{-5.9}$\\
    \bottomrule
  \end{tabular}%
  }
...
similarly in the bottom sector.  Its median width is sub-keV after the
physical-current normalization, although the mixing-angle scan leaves a long
upper tail.
...
Table~\ref{tab:literature-polished} places the preferred photon-normalized
results next to representative quark-model and phenomenological predictions
collected by Bondar and Milstein.  The comparison set contains early
nonrelativistic quark-model estimates, relativistic quark-model calculations,
more recent potential-model/electric-transition analyses, and the
Bondar--Milstein phenomenological extraction.  We cite the underlying papers
explicitly rather than citing only the summary table, since the spread among
these approaches is part of the physics context.  For channels with a vector
final state, $D_s^*\gamma$ or $B_s^*\gamma$, we quote the literature values in
the machine-readable table but leave our entry blank because those transitions
require separate LCSR correlators and are not part of the present calculation.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llllll}
    ...
    $\Dsone(2536)\to\Ds\gamma$
      & $15$ \cite{Godfrey2005DsPwave}
      & $25.2$--$31.1$ \cite{GoityRoberts2001}
      & $18.18$--$18.85$ \cite{Chen2020Electric}
      & $12$ \cite{BondarMilstein2025}
      & $0.504[0.127,1.104]$\\
    $\Dsone(2460)\to\Ds\gamma$
      & $6.2$ \cite{Godfrey2005DsPwave}
      & $10.3$--$17.2$ \cite{GoityRoberts2001}
      & $3.53$--$3.61$ \cite{Chen2020Electric}
      & $297$ \cite{BondarMilstein2025}
      & $16.1[12.4,20.5]$\\
    $\Bsone^{\rm low}(5750)\to\Bs\gamma$
      & $37$ \cite{LiNiZhong2021}
      & $70.6$ \cite{GodfreyMoatsSwanson2016}
      & $97.7$ \cite{LuPanWang2016}
      & $47$ \cite{BondarMilstein2025}
      & $18.4[12.5,28.1]$\\
    $\Bsone(5830)\to\Bs\gamma$
      & $27$ \cite{LiNiZhong2021}
      & $47.8$ \cite{GodfreyMoatsSwanson2016}
      & $56.6$ \cite{LuPanWang2016}
      & $28$ \cite{BondarMilstein2025}
      & $0.342[0.160,1.026]$\\
    \bottomrule
  \end{tabular}%
  }
  \caption{Representative literature comparison for the channels computed in
  this work.  Widths are in keV.  Our entries use the preferred lattice photon
  normalization.  A complete machine-readable table, including the uncomputed
  vector-final-state channels quoted in Ref.~\cite{BondarMilstein2025}, is
  stored in \texttt{outputs/bondar\_literature\_comparison\_table.csv}.}
  \label{tab:literature-polished}
\end{table}
...
high state near $5.829~\GeV$.  In our physical-current-normalized LCSR scan the lower
...
Thus the comparison is indirect: for $\Dsone(2460)$ we translate the measured
radiative branching fraction into the total-width scale implied by our partial
width; for $\Dsone(2536)$ we divide by the measured total width to estimate the
radiative branching fraction; and for $\Bsone(5830)$ we give a prediction
because no direct radiative measurement is available.  Table~\ref{tab:experiment-polished}
gives the corresponding interpretation for each case.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llll}
    ...
    $\Bsone(5830)\to\Bs\gamma$
      & legacy $\chi_s$: $1.33[0.612,3.60]~\keV$
      & no direct radiative measurement \cite{CMS2018}
      & future-search prediction\\
    $\Bsone(5830)\to\Bs\gamma$
      & lattice $f_{\gamma,s}^{\perp}$: $0.342[0.160,1.026]~\keV$
      & no direct radiative measurement \cite{CMS2018}
      & future-search prediction\\
    \bottomrule
  \end{tabular}%
...
against the pure axial-current benchmark.  The bottom result uses
